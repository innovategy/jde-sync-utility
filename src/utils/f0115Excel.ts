...
export interface F0115FieldDetail {
  fieldName: string;
...
export async function loadF0115FieldDetails(): Promise<
  Record<string, F0115FieldDetail>
> {
  const filePath = path.join(__dirname, '../../data/F0115.xlsx');
  ...
  let rowCount = 0;
  const fieldMap: Record<string, F0115FieldDetail> = {};
