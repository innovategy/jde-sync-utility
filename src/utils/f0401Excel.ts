...
export interface F0401FieldDetail {
  fieldName: string;
...
export async function loadF0401FieldDetails(): Promise<
  Record<string, F0401FieldDetail>
> {
  const filePath = path.join(__dirname, '../../data/F0401.xlsx');
  const workbook = new Workbook();

  // Check if file exists first
  if (!fs.existsSync(filePath)) {
    info('F0401.xlsx not found in data folder. Returning empty field map.');
    return {};
  }
  ...
  let rowCount = 0;
  const fieldMap: Record<string, F0401FieldDetail> = {};
